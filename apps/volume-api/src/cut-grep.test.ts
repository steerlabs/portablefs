...
// ---------------------------------------------------------------------------
// Cut-based grep on journal-served branches, over a REAL in-memory PFT2
...
describe("grep on journal-served branches", () => {
  test("mints a cut and answers matches when the branch has no ready cut yet", async () => {
  ...
  test("reuses the branch's newest ready cut regardless of age instead of minting", async () => {
    // The ready cut's boundary (5) is BEHIND the live journal position (7):
    // it still serves — grep's staleness is bounded by cut cadence, and a
    // fresh cut is minted only when NO ready cut exists.
    const world = await startCutServer({
      mode: "managed_journal",
      records: [
        readyRecord({ cutId: "hcut_old", resultCommitId: COMMIT_ID, cutSeqExclusive: "5" }),
      ],
    });
    const response = await postJson(world.baseUrl, "/v1/volumes/vol_a/grep", {
      pattern: "hello from",
    ...
    const body = (await response.json()) as GrepResponseBody;
    expect(body.matches).toEqual([{ file: "a/hello.txt", line: 1, text: "hello from pft2" }]);
    expect(body.cutId).toBe("hcut_old");
    expect(world.counters.snapshotCut).toBe(0);
  });

  test("newest-first: an incomplete ready record is skipped for an older complete one", async () => {
    // The newest record carries no result commit (its projection is
    // incomplete); the next-newest provable ready cut serves instead.
    const { resultCommitId: _omitted, ...incomplete } = readyRecord({
      cutId: "hcut_incomplete",
      resultCommitId: COMMIT_ID,
      createdAt: 9,
    });
    const world = await startCutServer({
      mode: "managed_journal",
      records: [
        readyRecord({ cutId: "hcut_older", resultCommitId: COMMIT_ID, createdAt: 5 }),
        incomplete,
      ],
    });
    const response = await postJson(world.baseUrl, "/v1/volumes/vol_a/grep", {
    ...
    expect(response.status).toBe(200);
    const body = (await response.json()) as GrepResponseBody;
    expect(body.cutId).toBe("hcut_older");
    expect(world.counters.snapshotCut).toBe(0);
  });
