...
export async function materializeManifest(
  root: string,
  manifest: TreeManifest,
  readBlob: (digest: BlobDigest) => Promise<Buffer>,
  options?: MaterializeOptions
): Promise<void> {
  const absoluteRoot = path.resolve(root);
  const concurrency = normalizeMaterializeConcurrency(options?.concurrency);
  await mkdir(absoluteRoot, { recursive: true });
  if (options?.prune ?? true) {
    await pruneWorkspace(absoluteRoot, manifest);
  }

  for (const entry of manifest.entries.filter((candidate) => candidate.kind === "directory")) {
    await mkdir(resolveEntryPath(absoluteRoot, entry.path), { recursive: true });
  }

  const nonDirectories: TreeEntry[] = [];
  for (const entry of manifest.entries) {
    const targetPath = resolveEntryPath(absoluteRoot, entry.path);
    ...
    nonDirectories.push(entry);
  }
  await runWithConcurrency(nonDirectories, concurrency, (entry) =>
    materializeNonDirectoryEntry(absoluteRoot, entry, readBlob, options)
  );
}

export async function materializeManifestDiff(
  root: string,
  diff: ManifestDiff,
  readBlob: (digest: BlobDigest) => Promise<Buffer>,
  options?: Pick<MaterializeOptions, "force" | "concurrency">
): Promise<void> {
  const absoluteRoot = path.resolve(root);
  const concurrency = normalizeMaterializeConcurrency(options?.concurrency);
  await mkdir(absoluteRoot, { recursive: true });
  ...
  for (const entry of removed) {
    await rm(resolveEntryPath(absoluteRoot, entry.path), { recursive: true, force: true });
  }

  const additionsAndChanges = [...diff.added, ...diff.changed];
  for (const entry of additionsAndChanges.filter((candidate) => candidate.kind === "directory")) {
    await materializeDirectoryEntry(absoluteRoot, entry);
  }
  await runWithConcurrency(
    additionsAndChanges.filter((candidate) => candidate.kind !== "directory"),
    concurrency,
    (entry) => materializeNonDirectoryEntry(absoluteRoot, entry, readBlob, options)
  );
}
...
async function pruneWorkspace(root: string, manifest: TreeManifest): Promise<void> {
  const keep = new Set(manifest.entries.map((entry) => entry.path));
  await pruneDirectory(root, "", keep);
}

async function materializeDirectoryEntry(root: string, entry: TreeEntry): Promise<void> {
  const targetPath = resolveEntryPath(root, entry.path);
  const info = await lstat(targetPath).catch((error) => {
    if ((error as NodeJS.ErrnoException).code === "ENOENT") {
      return null;
    }
  ...
  if (entry.mtimeMs > 0) {
    ...
    await utimes(targetPath, seconds, seconds).catch(() => undefined);
  }
}

async function materializeNonDirectoryEntry(
  root: string,
  entry: TreeEntry,
  readBlob: (digest: BlobDigest) => Promise<Buffer>,
  options?: Pick<MaterializeOptions, "force">
): Promise<void> {
  const targetPath = resolveEntryPath(root, entry.path);
  await mkdir(path.dirname(targetPath), { recursive: true });
  if (entry.kind === "symlink") {
    if (!options?.force && (await existingSymlinkMatchesEntry(targetPath, entry))) {
      return;
    }
    await rm(targetPath, { recursive: true, force: true });
  ...
  if (!entry.blob) {
    throw new Error(`File entry is missing blob: ${entry.path}`);
  }
  if (!options?.force && (await existingFileMatchesEntry(targetPath, entry))) {
    await chmod(targetPath, entry.mode).catch(() => undefined);
    if (entry.mtimeMs > 0) {
      const seconds = entry.mtimeMs / 1000;
      await utimes(targetPath, seconds, seconds).catch(() => undefined);
  ...
  if (entry.chunks?.length) {
    await materializeChunkedFile(targetPath, entry, readBlob);
  } else {
    const bytes = await readBlob(entry.blob.digest);
    const actualDigest = sha256Buffer(bytes);
    if (actualDigest !== entry.blob.digest) {
      throw new Error(`Blob checksum mismatch for ${entry.path}: ${actualDigest}`);
    }
    await writeFile(targetPath, bytes, { mode: entry.mode });
  }
  await chmod(targetPath, entry.mode).catch(() => undefined);
  if (entry.mtimeMs > 0) {
    const seconds = entry.mtimeMs / 1000;
...
async function existingSymlinkMatchesEntry(
  targetPath: string,
  entry: TreeEntry
): Promise<boolean> {
  const info = await lstat(targetPath).catch((error) => {
  ...
  return (await readlink(targetPath)) === (entry.linkTarget ?? "");
}

async function existingFileMatchesEntry(
  targetPath: string,
  entry: TreeEntry
): Promise<boolean> {
  if (!entry.blob) {
  ...
  return (await digestFile(targetPath)) === entry.blob.digest;
}

async function digestFile(filePath: string): Promise<BlobDigest> {
  const hash = createHash("sha256");
...
function normalizeMaterializeConcurrency(value: number | undefined): number {
  if (value === undefined) {
    return 8;
  }
...
async function pruneDirectory(
  ...
  const directoryPath = path.join(root, relativeDir);
  const children = await readdir(directoryPath, { withFileTypes: true }).catch((error) => {
    if ((error as NodeJS.ErrnoException).code === "ENOENT") {
      return [];
    }
    throw error;
  ...
  for (const child of children) {
    if (!relativeDir && child.name === volumeMetadataDirName) {
      continue;
    }
    const relativePath = toPosixPath(path.join(relativeDir, child.name));
    const absolutePath = path.join(root, relativePath);
    if (child.isDirectory()) {
      await pruneDirectory(root, relativePath, keep);
    }
...
async function materializeChunkedFile(
  targetPath: string,
  entry: TreeEntry,
  readBlob: (digest: BlobDigest) => Promise<Buffer>
): Promise<void> {
  if (!entry.blob || !entry.chunks?.length) {
    throw new Error(`Chunked file entry is incomplete: ${entry.path}`);
  }
  const handle = await open(targetPath, "w", entry.mode);
  const fullHash = createHash("sha256");
  let expectedOffset = 0;
  try {
    for (const chunk of [...entry.chunks].sort((left, right) => left.offset - right.offset)) {
      if (chunk.offset !== expectedOffset) {
        throw new Error(`Chunk offsets are not contiguous for ${entry.path}.`);
      }
      const bytes = await readBlob(chunk.digest);
      const actualDigest = sha256Buffer(bytes);
      if (actualDigest !== chunk.digest || bytes.byteLength !== chunk.size) {
        throw new Error(`Chunk checksum mismatch for ${entry.path}: ${actualDigest}`);
      }
      fullHash.update(bytes);
      await handle.write(bytes, 0, bytes.byteLength, chunk.offset);
      expectedOffset += bytes.byteLength;
    }
    if (expectedOffset !== entry.size) {
      throw new Error(`Chunked file size mismatch for ${entry.path}.`);
    }
    await handle.truncate(entry.size);
  } finally {
    await handle.close();
  }
  const actualFileDigest = `sha256:${fullHash.digest("hex")}`;
  if (actualFileDigest !== entry.blob.digest) {
    throw new Error(`Chunked file checksum mismatch for ${entry.path}: ${actualFileDigest}`);
  }
}
