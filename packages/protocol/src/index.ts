...
export const posixPathSchema = z
  ...
  .refine((value) => value.split("/").every((segment) => segment !== "" && segment !== "." && segment !== ".."), {
    // A tree-entry path is a sequence of real name components. Reject every
    // non-name segment at the boundary: an empty segment (`a//b`, a trailing
    // `/`), a `.`, and — the security-relevant one — a `..` parent escape.
    // volumePathSchema already rejects `..`; treeEntrySchema.path historically
    // used the laxer posixPathSchema, so a commit manifest could carry
    // `foo/../bar`. The Go PFT2 engine rejects "." / ".." DirEntry names
    // (pft2.go), so this simply moves the same guarantee to the wire boundary
    // instead of relying on the downstream reject.
    message: "Path segments must be real names (no empty, '.', or '..' segment)",
  })
