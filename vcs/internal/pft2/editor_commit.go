...
// touchedState resolves the transaction's coalesced effect: every inode whose
// stored object or index membership may change, and the per-parent directory
// edits that actually alter merged state. The plan build and the
// empty-outcome predicate both read it, so they can never disagree about what
// "this transaction changes nothing" means.
func (e *Editor) touchedState() (map[uint64]bool, map[uint64]map[string]*dirEdit) {
	...
	return touched, dirParents
}

// EmptyOverEmptyBase reports the one outcome Commit cannot express: the
// transaction has NO base filesystem root to carry forward and stages nothing
// that would create one, so there is no filesystem to publish and the plan
// build fails closed — an accidental-empty commit is a corruption signal on
// every ordinary path, and that guard never moves.
//
// History materialization is the one caller for which the state is
// legitimate: a cut whose record range carries only control state, over a base
// with no user filesystem root, reduces to the EMPTY filesystem. It proves the
// outcome with this predicate and then builds that filesystem explicitly
// (BuildEmptyFilesystem). Reading it never changes staged state and never
// affects committed bytes.
//
// Staged root xattr leaves also disqualify the empty outcome: a non-empty
// filesystem-homed xattr projection would ride the ROOT's XattrLeaves field,
// and the canonical empty filesystem carries none — so a transaction holding
// leaves keeps failing closed through Commit exactly as before. Setting an
// EMPTY projection (SetRootXattrLeaves(nil), what a control-only fold does)
// stages nothing and does not disqualify.
func (e *Editor) EmptyOverEmptyBase() bool {
	e.mu.Lock()
	defer e.mu.Unlock()
	if e.baseRoot != nil || len(e.rootXattrLeaves) > 0 {
		return false
	}
	touched, _ := e.touchedState()
	return len(touched) == 0
}

func (e *Editor) buildPlan(ctx context.Context) (*commitPlan, error) {
	touched, dirParents := e.touchedState()
