...
// ─── canonical empty filesystem ──────────────────────────────────────────────

// nodeSinkFunc adapts a function to NodeSink.
type nodeSinkFunc func(Ref, []byte) error

func (f nodeSinkFunc) PutNode(ref Ref, encoded []byte) error { return f(ref, encoded) }

// BuildEmptyFilesystem deterministically builds the CANONICAL EMPTY user
// filesystem — inode 1 as an empty directory (EmptyRootMode, nlink 1, no
// entries, epoch-zero timestamps), the one-entry inode index that pins it, and
// the ROOT over both — and returns it as the commit result of that
// filesystem. The three objects are a constant of the format: the same bytes
// every time, so the empty filesystem is part of a reduction's identity
// exactly like every other builder here.
//
// WHY THIS IS AN ENTRY POINT AND NOT A COMMIT PATH. An empty filesystem is a
// legitimate reduction OUTCOME: a history cut whose record range carries only
// control state, over a base with no user filesystem root, reduces to exactly
// this filesystem. An empty editor TRANSACTION over an empty base is not: it
// stages nothing and has no base to carry forward, which on every ordinary
// path is an accidental-empty commit, and buildPlan fails it closed. That
// guard stays. A caller that has PROVEN its outcome is the empty filesystem
// (Editor.EmptyOverEmptyBase) reaches this builder deliberately instead;
// nothing here inspects a transaction or relaxes anything.
func BuildEmptyFilesystem(nodes NodeSink) (*CommitResult, error) {
	if nodes == nil {
		return nil, invalidf("empty filesystem build requires a node sink")
	}
	out := &CommitResult{}
	sink := nodeSinkFunc(func(ref Ref, encoded []byte) error {
		if err := nodes.PutNode(ref, encoded); err != nil {
			return err
		}
		out.NewNodes++
		out.NewNodeBytes += int64(len(encoded))
		return nil
	})
	stage := func(n *Node) (Ref, error) {
	...
	rootInode, err := stage(&Node{Kind: KindInode, Inode: &Inode{
		Ino: RootIno, Kind: FileKindDirectory, Mode: EmptyRootMode, Nlink: 1,
	}})
	if err != nil {
		return nil, err
	}
	// The index is built through the same frozen construction rule every other
	// inode index takes, so the empty filesystem's index is byte-identical to
	// the one a transaction holding only inode 1 would commit.
	index, count, err := BuildInodeIndexTree([]InodeIndexEntry{{Ino: RootIno, Inode: rootInode}}, sink)
	if err != nil {
		return nil, err
	}
	facts := Root{
		RootInode: rootInode, InodeIndex: *index,
		MaxInoSeen: RootIno, InodeCount: count,
	}
	root, err := stage(&Node{Kind: KindRoot, Root: &facts})
	if err != nil {
		return nil, err
	}
	out.Root, out.RootFacts = root, facts
	return out, nil
}
