...
const (
	...
	RootIno = uint64(1)
	// EmptyRootMode is the mode of the root directory in the CANONICAL EMPTY
	// filesystem (BuildEmptyFilesystem). It is the same mode the shared
	// transition engine gives inode 1 on first touch of an empty base, so a
	// reduction that folds no tree intent at all and one that folds a no-op
	// intent agree on the root inode's bytes.
	EmptyRootMode = uint32(0o755)
)
