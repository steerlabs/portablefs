...
// Throttle patience for S3-compatible stores: 503 SlowDown / 429 is the
// store's flow control, not a failure (production: Railway object storage
// throttled a ~4,500-object upload burst mid-attempt). Retried per object
// with jittered exponential backoff, exactly like stock S3 SDK clients;
// bounded so a persistently throttling store still fails the attempt loudly
// (~30s worst case per object). Everything else stays attempt-fatal.
const (
	throttledRetries        = 6
	throttledInitialBackoff = 250 * time.Millisecond
	throttledMaxBackoff     = 8 * time.Second
)
...
// putThrottlePatient uploads one in-memory object, retrying ONLY the
// store's typed throttle answer within the bounded schedule above. The
// bytes are re-readable by construction, so a retry re-streams the exact
// same payload.
func putThrottlePatient(
	ctx context.Context,
	...
	backoff := throttledInitialBackoff
	for attempt := 0; ; attempt++ {
		err := store.Put(ctx, key, size, digestHex, bytes.NewReader(data))
		if err == nil || !errors.Is(err, histstore.ErrThrottled) || attempt >= throttledRetries {
			return err
		}
		if waitErr := throttleBackoff(ctx, backoff); waitErr != nil {
			return waitErr
		}
		if backoff < throttledMaxBackoff {
			backoff *= 2
		}
	}
}
...
func (s *cutStore) uploadOne(ref pft2.Ref, incarnation int64, data []byte) error {
	...
	for _, domain := range s.domains {
		...
		if err := putThrottlePatient(s.ctx, store, key, int64(ref.Size), ref.Hex(), data); err != nil {
...
// readbackVerified is ReadVerified with propagation AND throttle patience:
// it retries histstore.ErrNotFound (the just-written key not yet visible to
// readers) and histstore.ErrThrottled (the store's flow control) within the
// bounded backoff schedule above, so seconds of eventual consistency or
// rate limiting stop costing whole attempt restarts. Any other error, and
// absence/throttling that outlives the schedule, fail immediately.
func readbackVerified(ctx context.Context, store histstore.Store, key string, size int64, digestHex string) error {
	...
	for attempt := 0; ; attempt++ {
		...
		retryable := errors.Is(err, histstore.ErrNotFound) || errors.Is(err, histstore.ErrThrottled)
		if err == nil || !retryable || attempt >= readbackNotFoundRetries {
			return err
		}
		if waitErr := throttleBackoff(ctx, backoff); waitErr != nil {
			return waitErr
