...
// throttledPutStore answers the first refusals Puts with the store's typed
// rate-limit error — the production S3 SlowDown burst against a
// several-thousand-object upload.
type throttledPutStore struct {
	histstore.Store
	mu       sync.Mutex
	refusals int
}

func (s *throttledPutStore) Put(ctx context.Context, key string, size int64, digestHex string, body io.Reader) error {
	s.mu.Lock()
	if s.refusals > 0 {
		s.refusals--
		s.mu.Unlock()
		return fmt.Errorf("%w: put %s: HTTP 503: SlowDown", histstore.ErrThrottled, key)
	}
	s.mu.Unlock()
	return s.Store.Put(ctx, key, size, digestHex, body)
}
...
// TestUploadThrottleSucceedsWithinOneAttempt: a store answering 503
// SlowDown to a burst of PUTs (production: Railway object storage rate
// limiting) must be retried per object with backoff inside the attempt —
// one attempt, ready — never settled as an attempt-level failure.
func TestUploadThrottleSucceedsWithinOneAttempt(t *testing.T) {
	logBuf := &bytes.Buffer{}
	r := newRigWith(t, logBuf, nil, func(s histstore.Store) histstore.Store {
		if s.Domain() == domA {
			return &throttledPutStore{Store: s, refusals: 2}
		}
		return s
	})
	cut := buildManagedCut(t, "tenant-throttle", "hcut_throttle")
	r.repo.addCut(cut)
	...
	r.repo.mu.Unlock()
	if state != "ready" || attempts != 1 {
		t.Fatalf("throttling cost the attempt: state=%s attempts=%d (%s)", state, attempts, cut.lastError)
	}
	if findEvent(decodeEvents(t, logBuf), "cut_retry") != nil {
		t.Fatal("throttling was settled as an attempt-level retry")
	}
}
