...
func (cn *conn) dialOnce() error {
	...
	dialer := &net.Dialer{KeepAlive: connKeepAlive, Timeout: dialTimeout}
	var lastErr error
...
func dialPool(addr string, pool int, tlsCfg *tls.Config, auth func() string, transport func() (net.Conn, error)) (*Client, error) {
	...
	for i := 0; i < pool; i++ {
		cn := &conn{addrs: addrs, tls: tlsCfg, auth: c.tokenForHandshake, transport: transport, client: c}
		if err := cn.ensure(); err != nil {
			c.Close()
			return nil, err
		}
...
func (c *Client) Subscribe() (<-chan []coherence.Invalidation, error) {
	cn := &conn{addrs: c.addrs, tls: c.tls, auth: c.tokenForHandshake, client: c}
	if err := cn.ensure(); err != nil {
		return nil, err
	}
