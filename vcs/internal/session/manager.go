...
// FsyncAll makes every active session's recorded mutations durable on LOCAL
// media (WAL fsync), touching no network. This is the journal-first volume
// barrier used by unmount-class paths when the authority is unreachable: an
// acknowledged mutation is either already durable at the authority
// (write-through / an earlier flush) or in a session WAL, so after FsyncAll
// returns nil everything acknowledged survives even a machine crash and
// replays on the next clean start (RecoverAll).
func (m *Manager) FsyncAll() error {
	m.mu.Lock()
	sessions := make([]*Session, 0, len(m.byRoot))
	for _, s := range m.byRoot {
		sessions = append(sessions, s)
	...
	for _, s := range sessions {
		if err := s.FsyncLocal(); err != nil && firstErr == nil {
			firstErr = err
		}
	}
	return firstErr
...
// StopLocalDurable is Stop for the JOURNAL-FIRST unmount: it stops the
// background flusher and closes every session via CloseLocalDurable — WAL
// fsync + log close, NO final authority drain, NO checkin. The WALs stay on
// disk for the next clean start's recovery replay, and the un-checked-in
// checkouts are reclaimed by the authority's session-liveness machinery,
// exactly as after a crash.
func (m *Manager) StopLocalDurable() error {
	m.mu.Lock()
	if !m.stopped {
		m.stopped = true
		close(m.stop)
	...
	m.mu.Unlock()
	// Same quiesce as Stop: wait out any in-flight periodic tick so its
	// sweepIdle release cannot race these closes on the same WAL handle.
	m.runWG.Wait()
	m.mu.Lock()
	sessions := make([]*Session, 0, len(m.byRoot))
	for _, s := range m.byRoot {
	...
	var firstErr error
	for _, s := range sessions {
		if err := s.CloseLocalDurable(); err != nil && firstErr == nil {
			firstErr = err
		}
	}
	return firstErr
