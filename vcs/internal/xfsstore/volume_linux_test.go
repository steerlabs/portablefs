...
// TestCommitWriteDataRequirePositionAtEOF covers the one write shape a Linux
// frontend cannot resolve on its own: an offset that equals its kernel's i_size
// may be an ordinary positioned write or a per-call RWF_APPEND the kernel never
// forwarded. Only the true EOF, read inside the writer critical section, tells
// the two apart, so a disagreement is refused before anything is dispatched.
func TestCommitWriteDataRequirePositionAtEOF(t *testing.T) {
	v := openTestVolume(t)
	root, _ := v.Root()
	item, _, err := v.Create(root, "require-eof", 0o600, true)
	if err != nil {
		t.Fatal(err)
	}
	handle, err := v.OpenFile(item, OpenFlags{Read: true, Write: true})
	...
	payload := []byte("-tail")
	commit := WriteCommit{
		RequestedSize: uint64(len(payload)), Position: 6, RLimitSize: math.MaxUint64,
		FileMaxSize: math.MaxInt64, Mode: WritePositioned, RequirePositionAtEOF: true,
	}
	committed, assigned, post, err := target.CommitWriteData(payload, commit)
	if err != nil || committed != 5 || assigned != 6 || post.Size != 11 {
		t.Fatalf("write at EOF = (%d, %d, size %d, %v)", committed, assigned, post.Size, err)
	}

	stale := commit
	stale.Position = 6
	committed, _, _, err = target.CommitWriteData(payload, stale)
	if committed != 0 || !errors.Is(err, ErrPositionNotAtEOF) || !errors.Is(err, syscall.EIO) {
		t.Fatalf("write at a stale EOF = (%d, %v), want an exact EIO refusal", committed, err)
	}
	if errors.Is(err, ErrOutcomeUncertain) || errors.Is(err, ErrWritePostApply) {
		t.Fatalf("refusal before dispatch must be definite, got %v", err)
	}
	got := make([]byte, 12)
	if n, _ := v.ReadAt(handle, got, 0); n != 11 || string(got[:n]) != "prefix-tail" {
		t.Fatalf("refused write changed the object: %q", got[:n])
	}

	appendSpec := commit
	appendSpec.Position, appendSpec.Mode = 0, WriteAppend
	if _, _, _, err := target.CommitWriteData(payload, appendSpec); !errors.Is(err, fs.ErrInvalid) {
		t.Fatalf("append with an EOF requirement = %v, want ErrInvalid: an append resolves EOF itself", err)
	}
}
