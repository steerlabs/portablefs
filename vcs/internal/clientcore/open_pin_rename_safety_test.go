...
func TestDelegationReleaseBlocksWhenMarkOpenIsUnconfirmed(t *testing.T) {
	tests := []struct {
		name string
		reg  failingOpenRegistrar
	}{
		{
			name: "transport error",
			reg:  failingOpenRegistrar{err: errors.New("mark-open transport failure")},
		},
		{
			name: "definite refusal",
			reg:  failingOpenRegistrar{status: fsproto.EPERM},
		},
		{
			name: "inode disappeared",
			reg:  failingOpenRegistrar{status: fsproto.ENOENT},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			v := dialCore(t, serveCore(t), Options{
				Owner: "pin-failure-" + tt.name, VolumeID: "pin-failure-" + tt.name,
			...
			realRegistrar := v.openReg.reg
			v.openReg.reg = tt.reg
			err := v.wb.ReleaseFor(ctx, "d/held")
			v.openReg.reg = realRegistrar
			if err == nil {
				t.Fatal("delegation release succeeded without an authority-confirmed open pin")
			}

			status := v.wb.Status()
			if len(status.Delegations) != 1 || status.Delegations[0].Scope != "d" || !status.Delegations[0].Draining {
				t.Fatalf("failed pin did not keep the delegation held and draining: %+v", status.Delegations)
			}
			v.releasePinMu.Lock()
			pins := len(v.releasePins)
			v.releasePinMu.Unlock()
			if pins != 0 {
				t.Fatalf("failed mark-open recorded %d confirmed release pins, want 0", pins)
			}

			retryCtx, cancel := context.WithTimeout(ctx, 5*time.Second)
			defer cancel()
			if err := v.wb.ReleaseFor(retryCtx, "d/held"); err != nil {
				t.Fatalf("release did not recover after restoring the authority registrar: %v", err)
			}
		})
	}
}
...
func TestDirectoryRenameWhileOpenRekeysPinOwnership(t *testing.T) {
	...
	owner := openOwnerFor("root/new/held", n)
	v.releasePinMu.Lock()
	pin, ok := v.releasePins[owner]
	v.releasePinMu.Unlock()
	if !ok || pin.ino == 0 || pin.path != "root/new/held" {
		t.Fatalf("recall pin after directory rename = %+v, present=%v", pin, ok)
	}
	if _, st, err := v.client.GetattrOrphan(pin.ino); err != nil || st != fsproto.OK {
		t.Fatalf("renamed open inode was not parked: status=%d err=%v", st, err)
	}
	if !n.MarkOrphan(pin.ino, v.OpenOrphans()) {
	...
	v.releasePinMu.Lock()
	_, stillPinned := v.releasePins[owner]
	v.releasePinMu.Unlock()
	if stillPinned {
		t.Fatal("last close did not retire the rename-stable release pin")
	}
	awaitOrphanGone(t, v.client, pin.ino, "renamed open inode was not reaped after last close")
}
...
func TestOpenTrackerRenameUsesPathBoundaries(t *testing.T) {
	...
	tracker.Inc("old/child", moved)
	tracker.Inc("older/child", neighbor)
	...
	volume := &Volume{
		opens:       tracker,
		openReg:     registry,
		releasePins: map[openOwner]releasePin{openOwnerFor("old/child", moved): {ino: 10, path: "old/child"}},
	}
	...
	volume.releasePinMu.Lock()
	pin, ok := volume.releasePins[owner]
	volume.releasePinMu.Unlock()
