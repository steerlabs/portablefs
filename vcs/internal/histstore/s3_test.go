...
import (
	...
	"github.com/steerlabs/portablefs/vcs/internal/histstore/s3double"
)
...
func newS3(t *testing.T, pathStyle bool) (*histstore.S3Store, *s3double.Double) {
	...
	store, err := histstore.NewS3Store(histstore.S3Config{
		...
		// Retrying is on in production; these tests only need it bounded and
		// fast (the policy itself is proven in retry_test.go).
		Retry: histstore.RetryPolicy{MaxAttempts: 3, Base: time.Millisecond, Cap: 2 * time.Millisecond},
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestS3ServerErrorSurfacesTyped(t *testing.T) {
	...
	// A single 503 is throttling, not a failure: the store absorbs it.
	double.InjectFault(s3double.Fault{Method: "PUT", Status: 503, Remaining: 1})
	if err := store.Put(context.Background(), "err/object", 1, hexDigest(data), bytes.NewReader(data)); err != nil {
		t.Fatalf("one SlowDown failed the put: %v", err)
	}

	// Throttling that outlasts the bounded budget still surfaces as a
	// transport error (never as absence), for the cut-level retry to own.
	double.InjectFault(s3double.Fault{Method: "PUT", Status: 503, Remaining: -1})
	err := store.Put(context.Background(), "err/object", 1, hexDigest(data), bytes.NewReader(data))
	if err == nil || errors.Is(err, histstore.ErrNotFound) {
