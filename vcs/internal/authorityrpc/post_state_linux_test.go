...
func TestMutationPostStateExactObjectRoleSets(t *testing.T) {
	...
	tests := []struct {
		...
		{"create", &authoritypb.Request{Body: &authoritypb.Request_Create{}}, []postStateSnapshot{snapshot(target, postStateRoleCreated), snapshot(parent, postStateRoleParent)}, map[[16]byte]uint32{target: postStateRoleCreated, parent: postStateRoleParent}},
