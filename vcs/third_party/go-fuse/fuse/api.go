...
type RawFileSystem interface {
	...
	// Tmpfile creates an unnamed regular inode using the standard FUSE_TMPFILE
	// create input/output wire shape. name is the kernel's pinned slash-name.
	Tmpfile(cancel <-chan struct{}, input *CreateIn, name string, out *CreateOut) (code Status)
	...
	SyncFS(cancel <-chan struct{}, input *SyncFSIn) (code Status)
