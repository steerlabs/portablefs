...
func (c *rawBridge) SyncFS(cancel <-chan struct{}, input *fuse.SyncFSIn) fuse.Status {
	return fuse.ENOSYS
}
...
func (c *rawBridge) Tmpfile(cancel <-chan struct{}, input *fuse.CreateIn, name string, out *fuse.CreateOut) fuse.Status {
	return fuse.ENOSYS
}
