...
func (ms *Server) handleRequest(req *requestAlloc) Status {
	...
	errno := ms.writeReply(&req.request)
	if errno != 0 {
...
// ReplyWriteLifecycleArmed reports whether this real /dev/fuse server found
// the optional lifecycle on its RawFileSystem. A filesystem can check it from
// Init and refuse serving paths that cannot expose physical reply publication.
func (ms *Server) ReplyWriteLifecycleArmed() bool {
	return ms != nil && ms.replyWriteLifecycle != nil
}
