...
func TestFSKitCachePolicySelectsAnOperationalPolicyForMacOS26And27(t *testing.T) {
	tests := []struct {
		name          string
		version       string
		...
		{"ordinary macOS 27 synchronous repair", "27.0", "", portablefsd.V3CachePolicyMacOS26},
...
func TestFSKitCachePolicyDoesNotSelectNativeForUnqualifiedMacOS27Build(t *testing.T) {
	for _, qualification := range []string{"", "enabled", "sdk27"} {
		t.Run(qualification, func(t *testing.T) {
			policy, err := fskitCachePolicyForProductVersion(
				"27.0",
				qualification,
			)
			if err != nil {
			...
			if policy != portablefsd.V3CachePolicyMacOS26 {
				t.Fatalf("qualification %q selected policy %q, want synchronous repair", qualification, policy)
			}
		})
	}
}
