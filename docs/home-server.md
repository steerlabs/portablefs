...
- The PortableFS CLI wherever you will type `portablefs` commands:
...
```bash
git clone https://github.com/steerlabs/portablefs-oss.git
cd portablefs-oss
...
On macOS, `mount` uses the PortableFS FSKit extension — full POSIX semantics,
one transport, no fallback. One-time setup per Mac: install PortableFS.app and
